...
# Online implementation of stable fluids
# 2022
...
dens_prev = numpy.zeros([size, size])
dens = numpy.zeros([size, size])

u = numpy.zeros([size, size])
u_prev = numpy.zeros([size, size])

v = numpy.zeros([size, size])
v_prev = numpy.zeros([size, size])
...
for t in range(0, 1000):
    if(t<50):
        ...
        u[4, N // 2 - 1:N // 2 + 1] = 25
        dens[4, N // 2 - 1:N // 2 + 1] = indens

    vel_step(N, u, v, u_prev, v_prev, visc, dt)
    dens_step(N, dens, dens_prev, u, v, diff, dt)

    pyplot.imshow(dens.T, origin='lower', cmap="bone")
